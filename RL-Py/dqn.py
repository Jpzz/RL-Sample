...
game = 'Grid'
env_path = './../../Build/GridSample/{}'.format(game)
date_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
save_path = './save_models/{}/DQN/{}'.format(game, date_time)
load_path = './save_models/{}/DQN/20210514201212'.format(game)
...
class DQNAgent:
    def __init__(self):
        ...
        self.writer = SummaryWriter(log_dir=save_path, comment='DQNAgent')
    ...
    def train_model(self):
        ...
        q = (self.network(state) * one_hot_action).sum(1, keepdim=True)
        
        with torch.no_grad():
            next_q = self.target_network(next_state)
            target_q = reward + next_q.max(1, keepdim=True).values * ((1.0 - done) * discount_factor)
    ...
    def write_summary(self, score, loss, epsilon, step):
        ...
        self.writer.add_scalar('run/loss', loss, step)
        self.writer.add_scalar('run/epsilon', epsilon, step)
    ...
    for step in range(run_step + test_step):
        ...
        if done:
            ...
            if episode % save_interval == 0:
                ...
                print('Ep: {}, Step: {}, Score: {}, Loss: {}, Epsilon: {}'.format(episode, step, score, mean_loss, agent.epsilon))
